...
- By default, the AMA Data Collector will automatically upload the results to Application Modernization Accelerator when the scan has been completed
- If there is no network path from where the scan is executing to the AMA that the Data Collector came from then the upload will fail (not a hugh shock :) 
...
## Avoid seeing an upload error   
 ...
 - If there is no connection between the scanned system and AMA, the Data collector will throw an error when trying to upload. 
